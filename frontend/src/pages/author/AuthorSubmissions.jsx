    ...
    <div dir={i18n.dir()} className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        {[
          ...
          {
            label: "Pending",
            count: books.filter((b) => b.approvalStatus === "pending").length,
            color: "bg-amber-100 text-amber-700",
          },
          {
            label: "Approved",
            count: books.filter((b) => b.approvalStatus === "approved").length,
            color: "bg-green-100 text-green-700",
          },
          {
            label: "Rejected",
            count: books.filter((b) => b.approvalStatus === "rejected").length,
            color: "bg-red-100 text-red-700",
          },
        ].map((s) => (
          <div
            key={s.label}
            className={`px-4 py-2 rounded-xl text-sm font-semibold ${s.color}`}
          >
            {t(s.label)}: {s.count}
          </div>
        ))}
      ...
      {books.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {books.map((book) => {
            const currentStatus =
              book.approvalStatus || book.status || "pending";
                ...
                <div className="flex items-start gap-4 p-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="font-bold text-gray-900 dark:text-gray-200 text-base line-clamp-1">
                          {book.title}
                        </h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          {book.author || book.user?.name}
                        </p>
                        <p className="text-sm font-semibold text-indigo-600 dark:text-indigo-200 mt-1">
                          {book.price} {t("EGP")}
                        </p>
                      ...
                      {/* Status badge */}
                      <div
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-semibold flex-shrink-0 ${cfg.bg} ${cfg.text}`}
                      >
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      <button
                        onClick={() =>
                          navigate(`/author-dashboard/edit-book/${book._id}`)
                        }
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            <div className="bg-indigo-600 dark:bg-indigo-500 px-6 py-4 flex items-center justify-between">
              ...
              <button
                ...
                className="text-white/70 p-2 rounded-full bg-indigo-700 dark:bg-indigo-600 hover:text-white cursor-pointer text-xl"
              >
                ✕
              </button>
            </div>
              ...
              {contractData.signatureUrl && (
                <div className="rounded-xl p-4 bg-gray-50 dark:bg-gray-800">
                  <p className="text-sm font-semibold text-gray-600 dark:text-gray-200 mb-2">
                    {t("Your Digital Signature")}
                  </p>
                  <img
                    src={contractData.signatureUrl}
                    alt="Signature"
                    className="max-h-20 border border-gray-300 dark:border-gray-600 rounded-lg bg-white p-2"
                  />
                  {contractData.contractSignedAt && (
                    <p className="text-xs text-gray-400 dark:text-gray-200 mt-1">
                      {t("Signed on")}{" "}
                      {new Date(contractData.contractSignedAt).toLocaleString()}
                    </p>
                  )}
                </div>
              )}
